...
const updateSingleAcademicFacultyFromDB = async (_id : string,payLoad: Partial<TAcademicFaculty>) => {
  const result = await AcademicFaculty.updateOne({ _id }, payLoad,{new: true});
  return result;
};
